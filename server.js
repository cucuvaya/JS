...
function createComment(url, request) {
  ...
  const req_comment = req_body.comment || {};
  const body = req_comment.body;
  const username = req_comment.username;
  const articleId = req_comment.articleId;

  if (body === undefined
    || body.trim().length === 0 
    || database.users[username] === undefined
    || database.articles[articleId] === undefined) {
    const response = {
      status: 400
    }
    return response;
...
function deleteComment(url, request) {
  ...
  const articleComments = database.articles[articleId].commentIds;
  articleComments.splice(articleComments.indexOf(id), 1);

  const userComments = database.users[username].commentIds;
...
function upvoteComment(url, request) {
  ...
  const upidx = upvotedBy.indexOf(username);
  if (upidx === -1) {
    upvotedBy.push(username);
  }

  const downvotedBy = comment.downvotedBy;
  const downidx = downvotedBy.indexOf(username);  
  if (downidx !== -1) {
    downvotedBy.splice(downidx, 1);
  }
...
const databaseFile = 'database.yaml';
...
function loadDatabase() {
  ...
  const database = yaml.safeLoad(data);
  return database;
}
